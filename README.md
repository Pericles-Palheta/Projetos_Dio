# Projetos_Dio
Projetos desenvolvidos por Péricles Soares para o Bootcamp Excel,

NOTAS: 

PROJETO XBOX: O projeto XBOX não pôde ser finalizado pois as funções necessárias não estão disponíveis
no Excel Online, como mostram as capturas de tela em sua respectiva pasta. 

PROJETO IMPOSTO: Os botões de navegação e alguns campos de numeração não foram adicionados por limitações do Excel Online.

Ressalto que minha única opção para executar as tarefas do bootcamp foi usar a ferramenta online disponibilizada 
pela minha instituição de ensino pois não possuo a licença paga do Pacote Office.
